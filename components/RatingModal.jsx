...
const RatingModal = ({ ratingModal, setRatingModal }) => {
    ...
    const handleSubmit = async () => {
        if (rating < 0 || rating > 5) {
            return toast('Veuillez selectionner une note');
        }
        if (review.length < 5) {
            return toast('Ecrivez un petit avis');
        }
        ...
        <div className='fixed inset-0 z-120 flex items-center justify-center bg-black/10'>
            <div className='bg-white p-8 rounded-lg shadow-lg w-96 relative'>
                ...
                <textarea
                    ...
                    placeholder='Ecrivez votre avis (optionnel)'
                    rows='4'
                    value={review}
                    onChange={(e) => setReview(e.target.value)}
                ></textarea>
                <button onClick={e => toast.promise(handleSubmit(), { loading: 'Envoi...' })} className='w-full bg-orange-500 text-white py-2 rounded-md hover:bg-orange-600 transition'>
                    Envoyer la note
                </button>
            </div>
        </div>
