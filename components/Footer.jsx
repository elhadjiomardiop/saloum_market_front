    ...
    const sections = [
        {
            title: "Boutique",
            links: [
                { text: "Nouveautes", path: "/#nouveautes" },
                { text: "Meilleures ventes", path: "/#best-sellers" },
                { text: "Tous les produits", path: "/shop" },
                { text: "Creer ma boutique", path: "/create-store" },
            ],
        },
        {
            title: "Navigation",
        ...
        },
        {
            title: "Contact",
            links: [
                { text: "+221 77 000 00 00", path: "tel:+221771907788", icon: PhoneIcon },
                { text: "contact@saloummarket.sn", path: "mailto:contact@saloummarket.sn", icon: MailIcon },
                { text: "Nioro, Senegal", path: "https://maps.google.com", icon: MapPinIcon },
            ],
        },
    ];
    ...
    return (
        <footer className="mx-4 sm:mx-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col lg:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
                    <div>
                        <Link href="/" className="text-2xl sm:text-4xl font-semibold text-slate-700">
                            <span className="text-orange-600">Saloum</span>Market<span className="text-orange-600 text-5xl leading-0">.</span>
                        </Link>
                        <p className="max-w-[460px] mt-6 text-sm">
                            SaloumMarket rassemble des vendeurs de confiance et des produits verifies pour vous offrir une experience d'achat simple, rapide et securisee.
                        </p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-3 w-full lg:w-[55%] gap-8 text-sm">
                        {sections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-medium text-slate-700 mb-3 sm:mb-5">{section.title}</h3>
                ...
                <p className="py-4 text-sm text-slate-500">
                    Copyright {new Date().getFullYear()} © SaloumMarket. Tous droits reserves.
                </p>
            </div>
        </footer>
    );
